...
def shuffle_node_weights(graph, nodes=[]):
    ...
    if not nodes: nodes = list(graph.nodes)
    for u in nodes: u.set_weight(random.random())
...
class Annealer:
    ...
    def __init__(self, graph, movers,
        ...
        per iteration. The system temperature is kept constant across these steps. Defaults to 1.
        ...
        if not isinstance(self.movers, list): self.movers = [self.movers]
    ...
    def render(self, frame):
        ...
        if not self.render_per_frame: return
    ...
    def write(self, frame):
        ...
        rb_dict = self.graph.get_rb_dict()
...
class CrosslinkedLoopMover(Mover):
    ...
    def __init__(self, graph, restraints, label=""):
        ...
        for (u, v, sat) in self.graph.xl_data.values():
            if sat: continue
            if u.struct == "loop": self.nodes.append(u)
            if v.struct == "loop": self.nodes.append(v)
