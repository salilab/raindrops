...
class Trimmer(Select):
    ...
    def accept_residue(self, residue):
        if residue.id[0] != " ": return False # remove HETATMs
...
class Partition:
    ...
    def _get_ssa(self):
        ...
        for line in ssa_lines:
            l = line.split()
            chain, resid, struct = l[2], int(l[3]), l[5]
            if chain not in resdict: continue
            if chain not in ssa: ssa[chain] = []
            if resid not in resdict[chain]: continue
    ...
    def _get_chain_partition(self, ssa):
        ...
        for i in range(len(ssa)):
            if ssa[i].structure in SS:
                if begin is None: begin, end = i, i
                else: end = i
            else:
        ...
        for i in range(1, len(p_ss_)):
            if ssa[p_ss_[i][0]].resid - ssa[bracket[1]-1].resid - 1 <= self.min_loop:
        ...
        residues = [ssa[i].resid for i in range(len(ssa))]
        ss_string = "".join([ssa[i].structure for i in range(len(ssa))])
        ...
        for p in p_loop:
            ...
            if  looplen <= self.max_loop:
                ...
                if ssa[p[1]-1].resid - ssa[p[0]+n*self.max_loop-1].resid < self.min_loop:
...
class Node:
    ...
    def __init__(self, regions, weight=1.0, rb=1, struct=None):
        ...
        if self.struct not in NODE_STRUCT_TYPES:
            raise TypeError("Node structure type can only be one of ss, loop or seed.")
...
class Graph(nx.Graph):
        ...
        for t in self.topology:
            ...
            if rb is None:
                ...
                if rb not in range(1, self.maxrb+1):
                    raise ValueError("Rigid body id out of bounds. Should be limited to [1, %d]" % self.maxrb)
    ...
    def set_rbs(self, rbs, nodes=[]):
        ...
        if not nodes: nodes = list(self.nodes)
    ...
    def get_rb_dict(self):
        ...
        rb_dict = OrderedDict()
        for u in self.nodes:
            if u.rb not in rb_dict: rb_dict[u.rb] = []
