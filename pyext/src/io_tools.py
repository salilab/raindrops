...
class TopologyParser:
    ...
    def _read_topology_file(self, fn):
        ...
        for line in lines:
            ...
            src = spl[1].strip()
            if src == "BEADS":
                continue  # ignore flexible residues
            chain = spl[2].strip()
            resrange = spl[3].strip()
            pdb_offset = spl[4].strip()
...
class RigidBodyPdbWriter:
    ...
    def _write_rigid_body(self, rb, outdir):
        ...
        nodes = [u for u in self.rbdict[rb] if (u.struct != "loop")
                 or (u.weight >= self.min_loop_quality)]
        ...
        for molname in hier:
            ...
            for r in sorted(hier[molname]):
                ...
                for i, a in enumerate(list(src_atoms)):
                    sb.init_atom(
                        name=a.name, coord=a.coord, b_factor=0.0,
                        occupancy=1.0, altloc=" ", serial_number=i+1,
                        fullname=" " + a.name + " ", element=a.name[0])
...
class PMIStyleTopologyWriter:
    ...
    def __init__(self, graph, min_loop_quality=0.0):
        ...
        # get rb sizes
        self.rb_sizes = {k: len(v)
                         for k, v in self.graph.get_rb_dict().items()}
    ...
    def _get_components_from_molinfo(self, molname, molinfo):
        ...
        for next_r in residues[1:]:
            ...
            # tests to determine if these two residues are in the
            # same component
            is_contiguous_seq = next_r - curr_r == 1
            is_same_src = molinfo[next_r]["src"] == molinfo[curr_r]["src"]
            is_same_chain = \
                molinfo[next_r]["chain"] == molinfo[curr_r]["chain"]
...
def check_PMI_topology(topology_fn, rigid_body_pdb_dir, src_pdb_dir):
    ...
    for t in tp.topology:
        ...
        if src == "BEADS":
            continue
        if rb not in rb_resdict:
            rb_resdict[rb] = {}
        if chain not in rb_resdict[rb]:
            rb_resdict[rb][chain] = []
    ...
    for rb, chaindict in rb_resdict.items():
        ...
        for c, reslist1 in chaindict.items():
            if c not in all_chains:
                check = False
                status += (">Chain %s found in topology but "
                           "not in %d.pdb\n" % (c, rb))
                continue
            ...
            if reslist1 != reslist2:
                check = False
                status += (">Residue numbers of chain %s from topology "
                           "not found in %d.pdb\n" % (c, rb))
