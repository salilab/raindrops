...
def _get_graph(pdb_dir, topology_file):
    ...
    g = IMP.raindrops.protein_graph.Graph(
        tp, k=1, maxrb=10, symmetrize_edges=True)
    g.build()
    return g
...
class TestRaindrops(IMP.test.TestCase):
    ...
    def test_compactness_restraint(self):
        ...
        cr = IMP.raindrops.restraints.Compactness(
            g, label="test_compactness", weight=1.0)
        scores = cr.scores
    ...
    def _test_subgraph_connectivity_restraint(self):
        ...
        scr = IMP.raindrops.restraints.SubgraphConnectivity(
            g, label="test_subgraph_connectivity", weight=1.0)
        scores = scr.scores
    ...
    def test_crosslink_restraint_mapping(self):
        ...
        # crosslink restraint
        _ = IMP.raindrops.restraints.Crosslink(
